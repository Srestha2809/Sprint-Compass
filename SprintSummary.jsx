...
const SprintSummary = () => {
  ...
  const handleAddSubtask = () => {
    const subtasks = [...backlogState.form.subtasks, { description: "", actualhour: 0, reestimate: 0 }];
    setState({ form: { ...backlogState.form, subtasks: subtasks } });
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      <Card className="card">
        ...
        />
        <CardContent>
          <Button onClick={() => setState({ dialogOpen: true })}>Add Backlog</Button>
          <Table>
            <TableBody>
              {backlogState.backlogs.map((item) => (
                <TableRow key={item._id}>
                  <TableCell><DisplayBacklog data={item} refreshMembers={getBacklogs} /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
